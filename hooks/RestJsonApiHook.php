...
/**
 * RestJsonApiHook class help you to create custom web service on your wordpress
 */
class RestJsonApiHook {
    ...
    public static function hooks() {
        ...
        add_action('init', ['Rootpress\hooks\RestJsonApiHook', 'addRewriteRulesTag']);

    	// Router system for API
    	add_action('template_include', ['Rootpress\hooks\RestJsonApiHook', 'routerApiHook'], 11);

    	// Handle 404 for API
    	add_action('template_include', ['Rootpress\hooks\RestJsonApiHook', 'handle404'], 12);
     ...
     * Rooter api hook
     */
    public static function routerApiHook($template) {
    	global $wp_query;
    	...
    	if( basename($template, '.php') === 'page' && ( isset($wp_query->query) && isset($wp_query->query['pagename']) && ($wp_query->query['pagename'] == self::getApiKeyword()) )) {
    		...
    		if(is_null($action)) {
				...
				$actionMissing = apply_filters('rootpress_RestJsonApiHook_output_action_missing', $actionMissing);
    	...
    	if(basename($template, '.php') === '404' && strpos($_SERVER['REQUEST_URI'], '/' . self::getApiKeyword() . '/') === 0) {
    		...
    		if(is_null($apiEndpointPage)) {
    			...
    			echo json_encode([
					...
					'errors'  => [
						'code'    => false,
						'message' => 'Error: Api Endpoint Page not found. We create it for you. Just refresh the page to access your web hook.'
					]
				]);
    		}
    		else {
				...
				$error404 = apply_filters('rootpress_RestJsonApiHook_output_error_404', $error404);
	...
	private static function getApiKeyword() {
		...
		if(is_null(self::$prefixApiKeyword)) {
	    	...
	    	self::$prefixApiKeyword = apply_filters('rootpress_RestJsonApiHook_api_keyword', $defaultPrefixApiKeyword);
